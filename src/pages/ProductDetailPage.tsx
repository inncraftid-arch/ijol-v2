...
const ProductDetailPage = () => {
  ...
  const showPreviousImage = () => setActiveImage((current) => (current - 1 + galleryImages.length) % galleryImages.length);
  ...
  return (
    <>
      <section className="bg-white pb-20 pt-[calc(var(--height-navbar)+2.75rem)]">
        <Container>
          <nav className="flex flex-wrap items-center gap-2 text-sm text-brown/55" aria-label="Breadcrumb">
            <a href="/" className="transition hover:text-gold">Beranda</a>
            <span aria-hidden="true">/</span>
            <a href="/catalog" className="transition hover:text-gold">Koleksi</a>
            <span aria-hidden="true">/</span>
            <span className="font-bold text-gold">Gaudi Crop</span>
          </nav>
          ...
          <div className="mt-11 grid items-start gap-10 lg:grid-cols-[1fr_1.03fr] lg:gap-12">
            <div>
              <div className="relative overflow-hidden rounded-xl bg-cream">
                <img src={galleryImages[activeImage]} alt={product.title} className="aspect-[1/1.05] w-full object-cover" />
            ...
            <div className="pt-1">
              <h1 className="max-w-2xl text-2xl font-extrabold leading-snug text-brown md:text-[28px]">{product.title}</h1>
              ...
              <div className="mt-5 flex flex-wrap items-center gap-4 text-base text-brown/45">
                ...
                <span className="rounded-md bg-cream px-3 py-1.5 text-sm font-extrabold text-brown">Size: {product.size}</span>
                <span className="rounded-md bg-mist px-3 py-1.5 text-sm font-bold text-brown/35">{product.condition}</span>
              </div>

              <p className="mt-7 max-w-2xl text-lg leading-7 text-brown/80">
                Crop Gaudi warna kombinasi maroon dan cream, kondisi masih bagus dan sudah lama tidak dipakai.
              </p>

              <div className="mt-7 grid gap-5 sm:grid-cols-2">
                <div className="rounded-2xl border border-line bg-cream-soft px-6 py-4 text-center">
                  <strong className="block text-xl font-extrabold text-gold">Rp45.000</strong>
                  <span className="mt-1 block text-base text-brown/55">Preloved</span>
                </div>
                <div className="rounded-2xl border border-line bg-cream-soft px-6 py-4 text-center">
                  ...
                  <span className="mt-1 block text-base text-brown/55">Swap</span>
                </div>
              </div>

              <div className="mt-7 grid gap-4 sm:grid-cols-2">
                <button type="button" className="min-h-12 rounded-full border border-brown bg-white text-base font-bold text-brown transition hover:border-gold hover:text-gold">
                  Beli
                </button>
                <button type="button" onClick={() => openSwap(productIndex)} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-coral text-base font-bold text-white transition hover:brightness-95">
      ...
      <section className="bg-cream-soft py-16">
        <Container>
          <h2 className="font-display text-4xl font-normal tracking-normal text-brown">Pakaian Lain di Katalog</h2>
          <p className="mt-2 text-base text-brown/55">Outfit lain yang mungkin cocok dengan gayamu.</p>

          <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {Array.from({ length: 20 }, (_, index) => (
              <ProductCard key={index} image={assets.productItemJacket} index={0} href={`/catalog/${index + 1}`} />
            ))}
          </div>
        </Container>
      </section>
